...
pub fn part1() {
    let data = read_from_data_dir("day2.txt").unwrap();
...
/// Solves part 1. Returns the depth and horizontal results as a tuple.
fn movement_without_aim(instructions: String) -> (i32, i32) {
    let mut depth = 0;
    let mut horizontal = 0;
    for line in instructions.lines() {
        let num = get_number_from_line(String::from(line)).unwrap();
...
/// takes a string of the format `text num` and returns the num
/// For example
...
/// Solves part2. Returns the depth and horizontal results as a tuple.
fn movement_with_aim(instructions: String) -> (i32, i32) {
    let mut depth = 0;
    let mut horizontal = 0;
    let mut aim = 0;
    for line in instructions.lines() {
...
mod tests {
    ...
    fn test_movement_without_aim() {
        ...
        let (depth, horizontal) = movement_without_aim(instructions);
        assert_eq!(depth * horizontal, 150);
    }

    #[test]
    fn test_movement_with_aim() {
        let instructions = "forward 5\ndown 5\nforward 8\nup 3\ndown 8\nforward 2".to_string();
        let (depth, horizontal) = movement_with_aim(instructions);
        assert_eq!(depth, 60);
