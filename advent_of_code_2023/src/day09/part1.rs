use super::{create_diff_map, read_all_history};
...
pub fn solve(input: &str) {
    ...
    let mut forcasted_values = 0;
    for history in sensor_history.iter() {
        let mut diff_map = create_diff_map(history);
        extrapolate_next_diff_map(&mut diff_map);
        let forecasted_value = diff_map[0].last().unwrap();
        forcasted_values += forecasted_value;
    }

    println!("Sum of forecasted values: {}", forcasted_values);
}
...
mod tests {
    ...
    fn test_extrapolate_diff_map() {
        ...
        super::extrapolate_next_diff_map(&mut diff_map);
