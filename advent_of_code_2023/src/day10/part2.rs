...
use super::{part1::trace_pipe, read_map, Pipe, Position};
...
fn find_enclosed_tiles(map: &HashMap<Position, Pipe>) -> HashSet<Position> {
    let mut enclosed_tiles = HashMap::new();
    for (pos, pipe) in map.iter() {
        if pipe == &Pipe::Empty {
            enclosed_tiles.insert(pos.clone());
        }
    }
...
mod tests {
    ...
    fn test_find_enclosed_tiles() {
        ...
        let enclosed_tiles = find_enclosed_tiles(&map);
